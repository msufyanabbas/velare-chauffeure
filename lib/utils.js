...
// FIXED: Enhanced price calculation function with proper round trip handling
export const calculatePrice = async (bookingData) => {
  try {
    ...
    // Get distance and duration (ONE WAY only)
    const routeData = await calculateDistanceAndDuration(bookingData.pickupAddress, bookingData.dropoffAddress);
    const oneWayDistance = routeData.distance; // in kilometers (ONE WAY)
    const oneWayDuration = routeData.duration; // in minutes (ONE WAY)
    ...
    switch (serviceType) {
      case 'hourly':
        // Hourly booking calculation (round trip logic different for hourly)
        const actualHours = Math.max(hours, PRICING_CONFIG.hourly.minimumHours);
        breakdown.basePrice = vehicleRates.hourly * actualHours;
        
        // For hourly bookings, base price already covers the service regardless of round trip
        // Check if distance exceeds allowed kilometers
      ...
      case 'events':
        ...
        // FIXED: Apply round trip multiplier for events
      ...
      default:
        ...
        // FIXED: Apply round trip multiplier for base price
    ...
    const displayDistance = isRoundTrip ? oneWayDistance * 2 : oneWayDistance;
    const displayDuration = isRoundTrip ? oneWayDuration * 2 : oneWayDuration;
    ...
    return {
      ...
      distanceText: `${displayDistance.toFixed(1)} km${isRoundTrip ? ' (Round Trip)' : ''}`,
      durationText: `${Math.round(displayDuration)} min${isRoundTrip ? ' (Round Trip)' : ''}`,
      ...
      priceBreakdown: {
        ...
        distanceInfo: `${oneWayDistance.toFixed(1)} km${isRoundTrip ? ' × 2 (Round Trip)' : ''} (Base: ${serviceInfo.baseKmIncluded} km included)`,
        timeInfo: `${Math.round(oneWayDuration)} minutes${isRoundTrip ? ' × 2 (Round Trip)' : ''}`,
    ...
    const basePrice = (vehicleRates[serviceType] || vehicleRates.base) * (isRoundTrip ? 2 : 1);
    ...
    const timeCharges = estimatedDuration * 0.5;
