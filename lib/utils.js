// lib/utils.js - Fixed pricing structure with proper round trip calculations - 20% PRICE REDUCTION
...
// UPDATED: Pricing structure with 20% reduction (new prices are 80% of original)
const PRICING_CONFIG = {
  // Base rates for different vehicle types and services - REDUCED BY 20%
  baseRates: {
    luxury_sedan: {
      base: 68,           // Was 85 -> 68 (20% reduction)
      hourly: 124,        // Was 155 -> 124 (20% reduction)
      events: 144         // Was 180 -> 144 (20% reduction)
    },
    premium_luxury_sedan: {
      base: 88,           // Was 110 -> 88 (20% reduction)
      hourly: 132,        // Was 165 -> 132 (20% reduction)
      events: 168         // Was 210 -> 168 (20% reduction)
    },
    suv: {
      base: 88,           // Was 110 -> 88 (20% reduction)
      hourly: 132,        // Was 165 -> 132 (20% reduction)
    ...
    '7_seater': {
      base: 104,          // Was 130 -> 104 (20% reduction)
      hourly: 144,        // Was 180 -> 144 (20% reduction)
      events: 184         // Was 230 -> 184 (20% reduction)
    },
    'people_mover_11_seater': {
      base: 144,          // Was 180 -> 144 (20% reduction)
      hourly: 164,        // Was 205 -> 164 (20% reduction)
      events: 224         // Was 280 -> 224 (20% reduction)
    }
  },
  
  // Per km and per minute charges after base kilometers - REDUCED BY 20%
  perKmRate: 2.0,         // Was 2.5 -> 2.0 (20% reduction)
  perMinuteRate: 0.4,     // Was 0.5 -> 0.4 (20% reduction)
  
  // Base kilometers included in base price (unchanged)
  baseKilometers: 10,
  
  // Hourly booking configuration
  hourly: {
    minimumHours: 2,        // Minimum 2 hours booking (unchanged)
    baseKilometers: 80,     // 80 km per hour included (unchanged)
    excessKmRate: 2.8       // Was 3.5 -> 2.8 (20% reduction)
  },
  
  // Event booking configuration
  events: {
    baseKilometers: 15,     // More km included for events (unchanged)
    perKmRate: 2.4,         // Was 3.0 -> 2.4 (20% reduction)
    perMinuteRate: 0.6,     // Was 0.75 -> 0.6 (20% reduction)
    minimumDuration: 120    // Minimum 2 hours for events (unchanged)
  },
  ...
  try {
    ...
    if (window.google.maps.places && window.google.maps.places.Place) {
      ...
      const request = {
        ...
        locationBias: { lat: -33.9506, lng: 151.0893 }, // Beverly Hills, Australia
      };
...
export const calculatePrice = async (bookingData) => {
  try {
    ...
    if (bookingData.dropoffAddress && bookingData.dropoffAddress.trim()) {
      routeData = await calculateDistanceAndDuration(bookingData.pickupLocation.fullAddress, bookingData.dropoffLocation.fullAddress);
    ...
    // Enhanced fallback calculation with reduced prices
    const vehicleType = bookingData.vehicleType || 'luxury_sedan';
    const isRoundTrip = bookingData.isRoundTrip || false;
    const isHourlyTrip = bookingData.isHourlyTrip || false;
    const hours = bookingData.hours || PRICING_CONFIG.hourly.minimumHours;
    ...
    // Simple fallback calculation with reduced rates
    const distanceCharges = Math.max(0, (estimatedDistance - 10)) * 2.0; // Reduced from 2.5
    const timeCharges = serviceType === 'hourly' ? 0 : estimatedDuration * 0.4; // Reduced from 0.5
