...
export const calculatePrice = async (bookingData) => {
  try {
    ...
    let distance = routeData.distance; // in kilometers (ONE WAY)
    let duration = routeData.duration; // in minutes (ONE WAY)
    
    // DO NOT double the distance/duration here - calculate for one way first
    ...
    let breakdown = {
      ...
      timeCharges: 0,
      gst: 0,
      roundTripMultiplier: isRoundTrip ? PRICING_CONFIG.roundTripMultiplier : 1
    };
    
    // Calculate based on service type (ONE WAY calculation first)
    switch (serviceType) {
      case 'hourly':
        ...
        // For round trip hourly, apply multiplier to total
        if (isRoundTrip) {
      ...
      case 'events':
        ...
        // Round trip for events - apply multiplier to total
        if (isRoundTrip) {
          totalPrice *= PRICING_CONFIG.roundTripMultiplier;
          breakdown.basePrice *= PRICING_CONFIG.roundTripMultiplier;
          breakdown.kmCharges *= PRICING_CONFIG.roundTripMultiplier;
      ...
      case 'base':
      default:
        // Base booking calculation (ONE WAY first)
        totalPrice = basePrice;
        breakdown.basePrice = basePrice;
        ...
        // Round trip for base service - apply multiplier to total
        if (isRoundTrip) {
          totalPrice *= PRICING_CONFIG.roundTripMultiplier;
          breakdown.basePrice *= PRICING_CONFIG.roundTripMultiplier;
          breakdown.kmCharges *= PRICING_CONFIG.roundTripMultiplier;
    ...
    // For display purposes, show the actual distance/duration used in calculation
    const displayDistance = isRoundTrip ? distance * 2 : distance;
    const displayDuration = isRoundTrip ? duration * 2 : duration;
    ...
    return {
      ...
      priceBreakdown: {
        ...
        distanceInfo: `${distance.toFixed(1)} km${isRoundTrip ? ' × 2 (Round Trip)' : ''} (Base: ${serviceInfo.baseKmIncluded} km included)`,
        timeInfo: `${Math.round(duration)} minutes${isRoundTrip ? ' × 2 (Round Trip)' : ''}`,
