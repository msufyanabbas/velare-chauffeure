...
export const sendConfirmationEmail = async (to, booking, type) => {
  // Convert Mongoose document to plain object to avoid internal metadata
  const cleanBookingData = booking.toObject ? booking.toObject() : booking;
  ...
  // Create clean data object for post-ride email
  const postRideData = {
    ...cleanBookingData,
    postRideLink: `${process.env.NEXT_PUBLIC_BASE_URL}/post-ride?booking=${cleanBookingData.bookingId}&payment=${cleanBookingData.paymentIntentId}`
  };
  
  await emailService.sendTemplatedEmail(to, templateName, cleanBookingData);
  if(type == 'admin'){
  await emailService.sendTemplatedEmail(to, secondTemplateName, postRideData);
