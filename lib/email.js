...
export const sendConfirmationEmail = async (to, booking, type) => {
  ...
  console.log("booking for the post ride email is ", booking);
  await emailService.sendTemplatedEmail(to, templateName, booking);
  return emailService.sendTemplatedEmail(to, secondTemplateName, {...booking, postRideLink: `${process.env.NEXT_PUBLIC_BASE_URL}/post-ride?booking=${booking.bookingId}&payment=${booking.paymentIntentId}`});
};